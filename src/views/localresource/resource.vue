...
export default {
  ...
  mounted() {
    ...
    this.$r.get('/local_video_cate/0').then(re => {
      this.firstdata = re.data.result
    }).catch(errs => {
      console.log(errs)
    })
    this.$r.get('/local_video-cate').then(re => {
      this.iddata = re.data.result
    }).catch(errs => {
      console.log(errs)
    })
    this.$r.get('/local_video_series_query').then(re => {
  ...
  methods: {
    ...
    datachange(d) {
      ...
      this.seconddata = []
      if (d) {
        this.$r.get('local_video_cate/' + d).then(re => {
    ...
    querySearchAsync(queryString, cb) {
      this.$r.get('/local_video_series_query', { name: queryString }).then(re => {
    ...
    changedata(v, i) {
      ...
      if (v) {
        this.$r.get('local_video_cate/' + v).then(re => {
    ...
    creates() {
      if (this.formdatas.length <= 0 || this.seconddatas.length <= 0) {
        ...
        this.uploading = true
        this.$r.post('local_batchStore', { list: lists }).then(re => {
          if (re.data.status === 'success') {
